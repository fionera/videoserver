	...
	for _, v := range vs {
		...
		case amf.Object:
			...
			if tcurl, ok := obimap["tcUrl"]; ok {
				...
				connURL, err := url.Parse(connServer.ConnInfo.TcUrl + "/" + connServer.PublishInfo.Name)
				if err != nil {
					return err
				}
				connServer.connURL = connURL
			}
...
func (connServer *ConnServer) GetInfo() (app string, name string, url *url.URL) {
	app = connServer.ConnInfo.App
	name = connServer.PublishInfo.Name
	url = connServer.connURL
	return
}
