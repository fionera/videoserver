...
import (
	...
	rtmprelay2 "github.com/fionera/VideoServer/protocol/rtmprelay"
	"io"
	"log"
	"net"
	"net/http"
...
type ClientInfo struct {
	url              string
	rtmpRemoteClient *rtmp.Output
	rtmpLocalClient  *rtmp.Output
}

type Server struct {
	handler  av.Handler
	session  map[string]*rtmprelay2.RtmpRelay
	rtmpAddr string
}
...
func NewServer(h av.Handler, rtmpAddr string) *Server {
	return &Server{
		handler:  h,
		session:  make(map[string]*rtmprelay2.RtmpRelay),
		rtmpAddr: rtmpAddr,
	}
}
...
func (s *Server) handlePull(w http.ResponseWriter, req *http.Request) {
	...
	if oper[0] == "stop" {
		...
		pullRtmprelay := rtmprelay2.NewRtmpRelay(&localurl, &remoteurl)
...
func (s *Server) handlePush(w http.ResponseWriter, req *http.Request) {
	...
	if oper[0] == "stop" {
		...
		pushRtmprelay := rtmprelay2.NewRtmpRelay(&localurl, &remoteurl)
		log.Printf("rtmprelay start push %s from %s", remoteurl, localurl)
