...
type RtmpRelay struct {
	PlayUrl              string
	PublishUrl           string
	cs_chan              chan protocol.ChunkStream
	sndctrl_chan         chan string
	connectPlayClient    *protocol.ConnClient
	connectPublishClient *protocol.ConnClient
	startflag            bool
}
...
func NewRtmpRelay(playurl *string, publishurl *string) *RtmpRelay {
	return &RtmpRelay{
		...
		cs_chan:              make(chan protocol.ChunkStream, 500),
...
func (self *RtmpRelay) Start() error {
	...
	self.connectPlayClient = protocol.NewConnClient()
	self.connectPublishClient = protocol.NewConnClient()
